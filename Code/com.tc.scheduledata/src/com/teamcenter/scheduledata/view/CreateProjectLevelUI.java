...
public class CreateProjectLevelUI {
	...
	private static ArrayList<ModelObject> AllActiveProjList;
...
*/
	public CreateProjectLevelUI(TCSession session)
	{
		...
		try {
			...
			// Display display = new Display();
			 ...
			 // combobox projects
				...
				String[] projArr=new String[AllActiveProjList.size()];
				
				for(int i=0;i<AllActiveProjList.size();i++)
				{
					projArr[i]=AllActiveProjList.get(i).toString();
					ProjectList.put(AllActiveProjList.get(i).toString(), AllActiveProjList.get(i));
				}
   ...
   desc.setWidth(150);
	...
	protected static void setProgress_tableData(String selection, final SelectionEvent e) {
		try {
		    ...
		    //Dispose if already data is exist on grid
		...
		try {
			...
			//clean table
			 TreeItem[] tabItem = tableTree.getItems();
			    ...
			    //Set milestones in table as parent
			 ...
			 TreeItem[] mileitems = tableTree.getItems();
			  ...
			  {
				  for(TreeItem treeItem1:mileitems)
				  {
					  boolean flag=false;
					  ...
					  for(String key:tableAlldataList.keySet())
					  {
						  LinkedHashMap<String, String> taskData = tableAlldataList.get(key);
						   break_falg=false;
						  if(key.equals(treeItem1.getText()) )
						  {
							  flag=true;
							  parentItem=treeItem1;
		  ...
		  cildItem.setText(0,taskData.get("TASK_NAME"));
			...
			if(taskData.get("TASK_ASSIGNEE")!=null && taskData.get("TASK_ASSIGNEE").length()>0)
			{
				...
				/*if(resourceAssignee.contains("("))
				  ...
				  if(resourceAssignee.contains(","))
				  {
					  String users[] = resourceAssignee.split(",");
					  for(int u=0;u<users.length;u++)
		    		  {
						  resourceAssignee=obj.strValidation(users[u].trim());
						 if(u==0)
							 tempRes=resourceAssignee;
						  else
						  tempRes=tempRes+","+resourceAssignee;
						 resourceAssignee=tempRes;
		    		  }
				  }else if(resourceAssignee.equals("[]"))
				  {
					  resourceAssignee="";
				  }else
				  {
					  resourceAssignee=obj.strValidation(resourceAssignee.trim());  
				  }
